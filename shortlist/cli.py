"""Shortlist CLI — thin adapter over the engine; what the Phase-1 cron runs nightly.
...
def roster_slugs(
    ...
    """plex account id -> the slug Shortlist gave it. Assigned ONCE, never reassigned.
    ...
    * When a colliding account leaves, the survivor would revert from `name_2` to `name` — and
    ...
    if not path.exists() and existing_rows and not reseed:
        raise click.ClickException(
            f"{path} is missing but this server already has Shortlist rows. That file records which "
...
def main(ctx: click.Context, config_dir: Path, log_level: str) -> None:
    ...
    configure_logging(log_level, str(config_dir / "logs" / "shortlist.log"))
    ctx.obj = config_dir
