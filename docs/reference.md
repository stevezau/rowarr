...
| `recommendations.watched_pct`                         | `0.0`                              | max share of a row that may be already-finished titles (0 = all fresh, 1 = no filtering); per-row overridable. The "already-finished" set comes from each user's FULL play history, synced incrementally into a local store (Plex's history API only returns the most recent ~200 plays per call, so a heavy watcher's older watches used to slip back into rows). Note: a title _marked_ watched without ever being played isn't in any history source, so it can still appear — modern Plex logs a play event on mark-as-watched, so this shrinks over time. |
...
| `recommendations.recent_count`                        | `10`                               | how many of a person's most recent watches the `llm_web` source searches per row (one cached search each, "what to watch if you liked X"); results cached 14 days and shared across users so a popular title is searched once server-wide; per-row overridable (1–25)                                                                                                                                                                                                                                                                                          |
...
| `llm_web.search_provider`                             | `auto`                             | how the `llm_web` source searches: `native` (the curator's own web-search tool — Claude/GPT/Gemini only), `exa` (the Exa search API — every provider, including a local server), or `auto` (the default: the curator's own tool AND Exa **unioned** when both are set up — they surface mostly different titles — else whichever one)                                                                                                                                                                                                                          |
...
GET  /api/users/{id}/rows · PUT /api/users/{id}/rows/{collection_id} {muted?, row_size?}
...
GET  /api/runs · GET /api/runs/summary · GET /api/runs/{id} (each user carries `status`, `error`, `reason` — why a `skipped` user built nothing — and `has_trace`) · GET /api/runs/{id}/users/{user_id}/trace -> {username, display_name, status, trace} (the full per-user pipeline trace — history/seeds, each source's queries+returns, the web-search/RAG prompts, resolved vs. hallucinated titles; fetched on demand, `{}` on runs predating the feature) · GET /api/runs/{id}/log (activity feed) · POST /api/runs {user_ids?, collection_ids?, dry_run?} · POST /api/runs/{id}/cancel · DELETE /api/runs (clear all run history)
...
POST /api/report/sync -> 202 (kick off a watch-history sync so hit rates reflect the latest plays)
POST /api/tools/reconcile-watched -> {configured, users, added} (fill watch history from Plex's database — the only source that sees a mark-as-watched; reads read-only, writes nothing to Plex; `configured` false = no database mounted)
...
synced, uncapped — but _kind_: **Plex's history API returns playback sessions only and never returns
...
is not something anyone does by accident, so the mount _is_ the opt-in. `plex.db_path` overrides
...
(`mode=ro`, so SQLite reads the write-ahead log and gets a consistent snapshot of the live database
without ever writing to it), and Shortlist never modifies it. It only fills gaps: a title the play
history already covers is left alone, so flags never inflate the play counts the finished-show
fraction depends on.

Empty by default and never auto-detected — it requires a deliberate mount, and reading someone's
Plex database should be their choice. With Docker, mount it **read-only** (`:ro`):
...
**Reading it is a manual action, not part of every run.** A run never opens the Plex database:
scanning someone's live, multi-gigabyte production database every night — for marks, which most
people add rarely — is too heavy and too invasive to do automatically. Instead, **Tools → Reconcile
watched from Plex** runs it on demand and reports how many watched titles it found that the play
history had never seen. Run it once after mounting the database, and again whenever watched state
has drifted. Because the history API still can't see marks, anything marked watched _after_ a
reconcile stays invisible until the next reconcile.
