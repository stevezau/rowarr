...
from shortlist.server.db.models import CacheRow, RestrictionSnapshotRow, User
...
class DbSnapshotStore:
    ...
    def save(self, snapshot: FilterSnapshot) -> None:
        with self._sessions() as session:
            ...
            if user is None:
                # An account that shares the server but that Shortlist has never seen — someone the
